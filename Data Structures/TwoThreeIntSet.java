...
public class TwoThreeIntSet {
	...
	private boolean contains(Node n, int item) {
		...
		if (n.nodeType == 2) {
			...
			if (item<n.items[1] && item>n.items[0]) {
				return contains(n.subtrees[1],item);
			}
			return true;
			}
			...
			if (n.nodeType == 2) {
				...
				if (item<n.items[0]) {
					...
					if (result.nodeType!=4) {
						...
						Node tempL1 = new Node(result.items[0], result.subtrees[0],result.subtrees[1]);
						Node tempR1=new Node(result.items[2],result.subtrees[2],result.subtrees[3]);
						Node tempL=new Node(result.items[1],tempL1,tempR1);						
						Node tempR=new Node(n.items[1],n.subtrees[1],n.subtrees[2]);
					...
					if (result.nodeType!=4) {
						...
						Node tempL=new Node(n.items[0], n.subtrees[0], n.subtrees[1]);						
						Node tempL1=new Node(result.items[0], result.subtrees[0],result.subtrees[1]);
						Node tempR1= new Node(result.items[2],result.subtrees[2],result.subtrees[3]);						
						Node tempR=new Node(result.items[1], tempL1, tempR1);
					...
					if (result.nodeType!=4) {
						...
						Node tempL1 = new Node(result.items[0],result.subtrees[0],result.subtrees[1]);
						Node tempR1= new Node(result.items[2],result.subtrees[2],result.subtrees[3]);
						Node tempR=new Node(n.items[1],tempR1,n.subtrees[2]);
						Node tempL=new Node(n.items[0],n.subtrees[0],tempL1);
